...
from search import DoubleLayerSearcher, Searcher
import json
...
searcher = Searcher(device=device, adjacent_list=adjacent_list, road_center_gps=rid_gps)
...
for index, row in tqdm(true_traj.iterrows(), total=true_traj.shape[0]):
    ...
    with torch.no_grad():
        gen_trace_loc, gen_trace_tim = searcher.astar_search(road_generator, [rid_list[0]], [time_list[0]], rid_list[-1],
                                                             default_len=len(rid_list), max_step=10000)
